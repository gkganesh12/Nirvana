import { IsString, IsNotEmpty, IsOptional, IsObject, MaxLength, IsUrl, IsEmail } from 'class-validator';
...
export class IngestWebhookDto {
    ...
    @ApiProperty({ description: 'Detailed alert message', example: 'Service failed to connect to db:5432 after 3 retries' })
    @IsString()
    @IsNotEmpty()
    ...
    @ApiProperty({ description: 'Alert severity level', enum: ['INFO', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL'] })
    @IsString()
    @IsNotEmpty()
    @MaxLength(100)
    severity!: string;
    ...
    @ApiPropertyOptional({ description: 'Additional metadata tags', example: { host: 'node-1', region: 'us-east-1' } })
    @IsOptional()
    @IsObject()
    ...
    @ApiPropertyOptional({ description: 'Source URL of the alert', example: 'https://sentry.io/events/123' })
    @IsOptional()
    @IsString()
    @IsSafeUrl()
    @MaxLength(2048)
    ...
    @ApiPropertyOptional({ description: 'User associated with the event', example: 'admin@company.com' })
    @IsOptional()
    @IsString()
    @MaxLength(100)
    user?: string;
...
export class DatadogWebhookDto {
    ...
    @ApiPropertyOptional({ description: 'List of Datadog tags', example: ['service:web', 'env:prod'] })
    @IsOptional()
    @IsObject()
    @MaxJsonDepth(5)
    tags?: string[];
...
export class WebhookConfigDto {
    ...
    @ApiProperty({ description: 'Target URL for the webhook', example: 'https://hooks.slack.com/...' })
    @IsString()
    @IsUrl()
    @IsSafeUrl()
    @MaxLength(2048)
