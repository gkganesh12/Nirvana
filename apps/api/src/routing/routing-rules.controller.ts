...
import { CreateRoutingRuleDto, UpdateRoutingRuleDto, TestRuleDto } from '@signalcraft/shared';
...
export class RoutingRulesController {
  ...
  @Get()
  @RequirePermission(RESOURCES.ROUTING, 'READ')
  ...
  async getRule(@WorkspaceId() workspaceId: string, @Param('id') id: string) {
    return this.routingRulesService.getRule(workspaceId, id);
  }

  @Post()
  @RequirePermission(RESOURCES.ROUTING, 'WRITE')
  ...
  @Delete(':id')
  @RequirePermission(RESOURCES.ROUTING, 'DELETE')
  ...
  @ApiResponse({ status: 404, description: 'Rule not found' })
  async enableRule(@WorkspaceId() workspaceId: string, @Param('id') id: string) {
  ...
  @ApiResponse({ status: 404, description: 'Rule not found' })
  async disableRule(@WorkspaceId() workspaceId: string, @Param('id') id: string) {
  ...
  async reorderRules(@WorkspaceId() workspaceId: string, @Body() body: { ruleIds: string[] }) {
