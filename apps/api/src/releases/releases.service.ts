...
export class ReleasesService {
  ...
  async createRelease(workspaceId: string, data: CreateReleaseDto) {
    const release = await prisma.release.upsert({
      where: {
        workspaceId_version_environment_project: {
          workspaceId,
  ...
  async updateRelease(workspaceId: string, releaseId: string, data: UpdateReleaseDto) {
  ...
  async deleteRelease(workspaceId: string, releaseId: string) {
    const existing = await prisma.release.findFirst({
      where: { id: releaseId, workspaceId },
    });
  ...
  async linkAlertToRelease(
    ...
    if (!release) {
      release = await this.createRelease(workspaceId, { version, environment, project });
    }
