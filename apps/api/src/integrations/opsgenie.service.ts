...
export class OpsgenieService {
  ...
  async createAlertFromAlert(
    ...
    return this.createAlert(workspaceId, {
      message: alert.title,
      description: `SignalCraft alert ${alert.id} - ${alert.title}`,
      priority: this.mapToPriority(alert.severity),
      alertId: alert.id,
      source: 'SignalCraft',
    });
  }

  async acknowledgeAlert(workspaceId: string, alertId: string) {
    await this.updateAlertStatus(workspaceId, alertId, 'acknowledge');
  }

  async closeAlert(workspaceId: string, alertId: string) {
    await this.updateAlertStatus(workspaceId, alertId, 'close');
  }
  ...
  private async updateAlertStatus(
    ...
    if (!integration || !integration.configJson) {
      this.logger.warn('Opsgenie not configured', { workspaceId });
      return;
    }
    ...
    await axios.post(
      `${apiUrl}/alerts/${alertId}/${action}`,
      { note: `SignalCraft ${action}` },
      {
        headers: {
          Authorization: `GenieKey ${config.apiKey}`,
          'Content-Type': 'application/json',
