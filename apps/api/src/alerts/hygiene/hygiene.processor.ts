...
interface HygieneJobData {
  type: 'auto-close' | 'expired-snoozes' | 'anomaly-detection';
...
export class HygieneProcessor implements OnModuleDestroy {
  ...
  private async processJob(job: Job<HygieneJobData>) {
    ...
    switch (type) {
      ...
      case 'anomaly-detection':
        await this.processAnomalyDetection(workspaceId);
        break;
      default:
  ...
  private async processAnomalyDetection(workspaceId?: string) {
    const workspaces = workspaceId
      ? [{ id: workspaceId }]
      : await prisma.workspace.findMany({ select: { id: true } });

    for (const ws of workspaces) {
      const anomalies = await this.anomalyDetectionService.detectAndRecordAnomalies(ws.id);
      if (anomalies.length > 0) {
        this.logger.warn(`Detected ${anomalies.length} anomalies`, { workspaceId: ws.id });
      }
    }
  }
}
