...
export class HygieneController {
  ...
  async unsnoozeAlert(@WorkspaceId() workspaceId: string, @Param('id') alertGroupId: string) {
  ...
  @ApiResponse({ status: 404, description: 'Alert not found' })
  async resolveAlert(@WorkspaceId() workspaceId: string, @Param('id') alertGroupId: string) {
  ...
  async acknowledgeAlert(@WorkspaceId() workspaceId: string, @Param('id') alertGroupId: string) {
