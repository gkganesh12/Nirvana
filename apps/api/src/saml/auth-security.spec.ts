...
describe('Auth Security Testing', () => {
    ...
    describe('SAML Security', () => {
        it('should throw ValidationException for empty SAML response', async () => {
            await expect(samlService.validateAssertion('', 'cert', 'id')).rejects.toThrow(ValidationException);
        });
        ...
        it('should throw AuthenticationException for invalid signature format', async () => {
            ...
            const invalidXml = '<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol">Invalid</samlp:Response>';
            const base64Response = Buffer.from(invalidXml).toString('base64');

            await expect(samlService.validateAssertion(base64Response, 'fake-cert', 'idp-1')).rejects.toThrow(AuthenticationException);
        });
    });
