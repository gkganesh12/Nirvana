...
export class DashboardService {
  ...
  private async getDeduplicationRatio(
    ...
    const currentRatio =
      currentEvents > 0 ? Math.round((1 - currentGroups / currentEvents) * 100) : 0;
    const previousRatio = prevEvents > 0 ? Math.round((1 - prevGroups / prevEvents) * 100) : 0;
   ...
   * Get top noisy sources (projects with most alerts)
   */
  private async getTopNoisySources(workspaceId: string, since: Date): Promise<TopSource[]> {
   ...
   */
  private async getIntegrationHealth(workspaceId: string): Promise<IntegrationHealthStatus[]> {
  ...
  private getIntegrationName(type: IntegrationType): string {
    switch (type) {
      ...
      case IntegrationType.SENTRY:
        return 'Sentry';
      case IntegrationType.AWS_CLOUDWATCH:
        return 'AWS CloudWatch';
      case 'AZURE_MONITOR' as IntegrationType:
        return 'Azure Monitor';
      case 'GCP_MONITORING' as IntegrationType:
        return 'GCP Monitoring';
      case IntegrationType.PROMETHEUS:
        return 'Prometheus';
      case IntegrationType.GRAFANA:
        return 'Grafana';
      case IntegrationType.GENERIC_WEBHOOK:
  ...
  async getAnalytics(workspaceId: string, range: string) {
    ...
    for (let i = bucketCount - 1; i >= 0; i--) {
      ...
      const count = alertGroups.filter(
        (ag) => ag.createdAt >= bucketStart && ag.createdAt < bucketEnd,
      ).length;
    ...
    const resolutionRate =
      totalAlerts > 0 ? Math.round((resolvedAlerts / totalAlerts) * 1000) / 10 : 0;
